...
if(isset($_POST['report'])) {
    ...
    //$search = $_POST['search'];
    $query = "
        SELECT  c.carerid carerid, concat(c.firstname,' ', c.lastname) as carername, ch.DateFrom DateFrom, ch.DateTo DateTo, ch.NoOfDays NoOfDays,
         ch.reason reason, concat(ad.firstname,' ', ad.lastname) as adminname FROM tblcarer c,
        tblcarerholiday ch, tbladmin ad where c.CarerID=ch.CarerID and ad.AdminID = ch.ApprovedByAdminID and
        (c.firstname like '%$search%' or c.lastname like '%$search%' or ch.DateFrom='".date('Y-m-d',strtotime($search))."' or ch.DateTo='".date('Y-m-d',strtotime($search))."' or
        ch.NoOfDays = '$search' or ch.status LIKE '%".strtoupper($search)."%')";
}else
{
    //echo "Eles part";
    $query = "
        SELECT  c.carerid carerid, concat(c.FirstName,' ', c.LastName) as carername, ch.DateFrom DateFrom, ch.DateTo DateTo,
        ch.NoOfDays NoOfDays, ch.reason reason , concat(ad.FirstName,' ', ad.LastName) as adminname FROM tblcarer c, tblcarerholiday ch,
        tbladmin ad where c.CarerID=ch.CarerID and ad.AdminID = ch.ApprovedByAdminID";
}
        ...
        <div class="">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="x_panel">
                <div class="x_title">
                  <h2>Holiday Report <!--<small>Sample user invoice design</small>--></h2>
                            ...
                            while($row =mysqli_fetch_array($res, MYSQLI_ASSOC))
                            {
                                if($count%2 == 0)
                                {
                                    ...
                                    <tr class="even pointer">
                                        ...
                                        <td class=" "><?php echo htmlentities($row['adminname'], ENT_QUOTES, 'UTF-8'); ?></td>
