...
<?php 
   ...
   if(!empty($_GET)) 
    { 
       //$carerholidayid = $_GET["carerholidayid"];
        { 
            // Define our SQL query 
            $query = " 
                Update tblcarerholiday SET  
                  approvedon = now(), approvedbyadminid = :adminid
            ";

   	 	$query .= " WHERE carerholidayid = :carerholidayid"; 
...
<h1>Approve Carer Holiday Request</h1> <br>
<a href=admin_manage_carers.php> Manage Carers</a>  |  |  <a href=admin_view_carer_holidays.php> View Approved Holidays</a>
...
<table> 
    ...
    <?php foreach($rows as $row): $i = $i + 1;?> 
	    ...
	    <td><a href=admin_approve_holidays.php?carerholidayid=<?php echo $row['carerholidayid']; ?>> Approve Holiday</a></td>
